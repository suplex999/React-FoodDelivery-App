...
const SignUpSection = () => {
  ...
  const {
    ...
    reset, // ✅ ADD for form reset
  } = useForm();

  // In SignUpSection.jsx - REPLACE onSubmit only
  ...
  // const onSubmit = async (data) => {
  //   try {
  //     // ✅ FIXED: Correct backend endpoint + only email/password
  //     const res = await axios.post("http://localhost:5000/api/auth/register", {
  //       email: data.email,
  //       password: data.password,
  //     });
  ...
  //     // ✅ Success feedback + redirect to HOME (not login)
  //     alert("Account created successfully! Welcome to FoodKart! 🎉");
  //     reset(); // Clear form
  //     navigate("/"); // Go to home/products

  //   } catch (error) {
  //     console.error("Signup error:", error);
  //     // ✅ Better error handling matching backend
      ...
      <div className="w-full max-w-6xl grid gap-10 lg:gap-14 md:grid-cols-2 items-center">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border-t-4 border-black px-5 sm:px-7 lg:px-8 py-7 sm:py-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            {/* Name - Keep for UI but ignore in API */}
            <div>
              <input
                type="text"
